...
用户偏好：
		...
		2.当且仅当用户明确批准“现在进入全自动模式”时，则进入全自动模式，即：在smoke过程中发现的blocker自动拟定修改方案，可以不依赖用户通过子智能体修改并批准方案，代码审查后自动提交pr，达到p0=0 p1=0后保留pr不与远端合并，但是本地完成归档操作，并继续使用修改好的新代码分支继续下一轮最小预算，例如 design.n=3测试——直到smoke全通，最后向用户回报测试全通，并退出全自动模式。
