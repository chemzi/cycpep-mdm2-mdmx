...
def _require_mdm_reference_route(route_name):
    target_ids = set(required_target_ids(config.ACTIVE_PROJECT_CONFIG))
    if target_ids != {"MDM2", "MDMX"}:
        raise RuntimeError(
            f"{route_name} contains MDM-specific motif knowledge and is disabled for "
            f"project {config.ACTIVE_PROJECT_CONFIG['project_id']}; provide project-specific motifs instead"
        )
...
def _merge_config(target_spec, design_config):
    ...
    ts = target_spec or {}
    dc = design_config or {}
    project = config.ACTIVE_PROJECT_CONFIG
