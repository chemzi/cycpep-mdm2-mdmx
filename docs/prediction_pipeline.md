...
## Execution-owned initial Prediction

Launcher initial Prediction is generated by the registered
`evaluate_new_design_candidates` Execution action and published through its
transaction-managed ingest. `agents.prediction.run()` remains the
direct/non-Launcher ingest/evaluate seam, not a heavy scientific materializer.

Formal trace `run_id` is always the Orchestrator run; Prediction domain identity
is `prediction_run_id`. Execution readiness reuses the existing Prediction
