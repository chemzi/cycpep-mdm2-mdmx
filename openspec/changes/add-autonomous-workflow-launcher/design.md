...
The first launch also serializes an internal `RuntimeLocatorBinding` into the initial diagnostic before Research. It contains the already resolved data directory, Evidence directory, formal database path, approved project path, and `execution_root` needed to reconstruct the same `ProjectContext` and transaction staging location in another process. `status` and `resume` load this binding first and construct the context from it without re-reading `CYCPEP_DATA_DIR`, `CYCPEP_EVIDENCE_DIR`, `CYCPEP_DB_PATH`, `CYCPEP_EXECUTION_ROOT`, `NP_DATA`, or repository defaults for those bound locations. Missing, malformed, conflicting, or inaccessible bindings fail closed. The binding is location metadata only: it selects the Store, project contract, and Execution staging root to query but never asserts boundary completion. Its path values are internal and are omitted from `BrowserResult` and generic Frontend Evidence presentation.

Launcher resolves the initial `execution_root` once through the existing `ExecutionConfig.from_environment()` contract before the durable sidecar is written. On later commands it constructs the normal current-environment `ExecutionConfig` for all other tool settings and replaces only `execution_root` with the durable value. `DefaultWorkflowRuntime` passes that same restored config to `inspect_transaction_recovery`, `ensure_transaction_recovery_clean`, and `drain_run`. No environment mutation or second ExecutionConfig parser is introduced; standalone Worker and non-Launcher execution continue using `ExecutionConfig.from_environment()` unchanged.

Context restoration uses a Store-owned read-only validator to require both the original database file and its required formal schema plus expected project binding before constructing `DefaultWorkflowRuntime`. It never uses SQLite's create-on-connect, schema initialization, or migration behavior as recovery. A crash after the diagnostic/sidecar pair but before the Store was fully initialized therefore produces the same structured unavailable blocker as a later missing Store; the launcher cannot distinguish those cases from formal state and must not infer that Research is safe to start.
