...
- Durably bind the resolved data, Evidence, database, approved-project, and Execution staging-root locators in the initial internal diagnostic before science, then reuse that exact locator set for later `status` and `resume` commands so ambient path drift cannot select a different formal Store or transaction marker root. These locators remain internal location metadata and never prove workflow completion.
...
- Require later commands to open the original formal Store without creating or initializing a replacement database; a missing original Store is a structured recovery blocker before runtime construction or scientific inspection.
...
- **Affected areas:** new `workflow/` coordination and CLI modules, narrow public Agent/Store read-only boundary additions, tests, and operator documentation.
