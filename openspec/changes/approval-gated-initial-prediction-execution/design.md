...
The canonical PRODIGY identity is the installed distribution version `2.4.0`, matching deployed package metadata exactly. Runtime observation must exact-match that value; abbreviated aliases such as `2.4` are not formal identity. `ExecutionConfig.from_environment()` may skip an inaccessible hardcoded fallback on `PermissionError` or `OSError`, but it preserves an explicitly selected venv Python entrypoint without symlink canonicalization.

Formal Prediction publication proof includes the exact record Artifact set and, for each exact-scope candidate, exactly one transaction-bound `prediction_recorded` and one `battery_evaluated` event, plus exactly one `prediction_handoff_ready` event. All share the complete execution trace, Prediction run identity, and execution identity. The validator returns all authoritative Evidence IDs for Launcher recovery.

Retry inspection and retry-plan creation call one shared read-only validator for the same explicitly retryable terminal transaction and matching failure Evidence. Missing, active, `COMMITTING`, `COMMITTED`, compensation-conflict, unknown, or mismatched proof fails closed at both call sites.

PR71 cache retry remains compatible by constructing cached `PredictionPipeline` runs with the same explicit execution identity. Production identity requirements are not optionalized or inferred from cached files.
