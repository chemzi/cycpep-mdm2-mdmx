...
- **Public interfaces:** Planner gains an additive formal bootstrap-plan entry point/plan variant, and Launcher resume gains an explicit bootstrap-Prediction retry request for a terminal failed execution. Newly generated Prediction execution tasks gain a required path-independent execution identity. The existing `agents.prediction.run()` signature remains compatible. Existing Critic-driven plan source semantics and scientific behavior remain unchanged.
- **Data formats:** The Planner plan contract gains a versioned bootstrap source variant bound to Initial Design completion and committed candidate scope, plus a protocol/configuration identity that is independent of deployment paths. Existing plan, approval, task, transaction, Artifact, Evidence, and Prediction record formats are reused wherever possible; no Store schema changes.
