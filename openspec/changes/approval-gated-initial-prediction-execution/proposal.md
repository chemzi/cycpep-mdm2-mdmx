...
- Close the post-PR71 integration blockers without changing that architecture: use deployed PRODIGY package version `2.4.0` as canonical identity, require one transaction-bound `battery_evaluated` Evidence per exact-scope candidate during formal recovery, share one retryable-terminal proof validator between retry recovery and retry creation, tolerate inaccessible hardcoded Python fallbacks while preserving venv entrypoints, and keep cached Prediction execution identity explicit.
