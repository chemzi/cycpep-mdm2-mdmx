...
Write capability is evaluated from an existing path or nearest existing parent and OS access metadata. Doctor does not create a probe file or directory. An existing checkpointed SQLite file must pass `validate_storage_backend` through an immutable read snapshot so opening it cannot touch `-shm`/`-wal` sidecars; any non-empty WAL fails closed because SQLite immutable mode ignores WAL files. The Store owner must provide a quiescent `wal_checkpoint(TRUNCATE)` boundary before preflight. Ordinary Store readers keep their existing WAL-aware locking behavior. An absent fresh database is not opened or created and instead passes only when its explicit target and nearest existing parent are usable. GPU visibility uses a bounded `nvidia-smi` query and records model/driver/memory observations without running CUDA kernels.
