...
Successful completion validation requires the completion receipt to carry a non-empty transaction identifier. That transaction must be `COMMITTED`, and the authoritative `candidate_registered` identifiers in that exact transaction must equal the completion candidate identifiers as a set with no missing, extra, malformed, or duplicate values. A non-empty pre-transaction completion is not accepted as compatibility state; it remains `design_recovery_ambiguous`.

Before strict RFdiffusion starts its subprocess, remove every PDB matching only that invocation's output-prefix pattern. The existing failure cleanup remains in place. This guarantees the post-launch expected-output check observes files from the current subprocess rather than a prior run with the same target and seed. The non-strict route is outside this final scope.
