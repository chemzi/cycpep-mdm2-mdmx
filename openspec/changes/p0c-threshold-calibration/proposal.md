...
- **D3 Threshold persistence（修改）**
  - 标定结果通过正式 Store 持久化：校准产物注册到 `artifacts` 表，formal evidence 事件完整记录，阈值状态经 SQLite 事务路径写入（`State.update_state`/`replace_state`）。
  - JSON 缓存（`_threshold_calibration.json` / `_threshold_cache.json`）降级为 projection / compatibility surface，不再作为正式写入入口。
- **D4 未标定指标（新增）**
...
- 实现：`threshold_calibration.py`、`agents/research.py`、`data_layer.py`、`battery_evaluation.py`、`scripts/calibrate_thresholds.py`、新增对照评分脚本、`benchmarks/keap1`。
- 公开接口：`calibrate_thresholds` / `load_control_dataset` 保持兼容；control dataset schema 版本递增；可能新增可选参数，默认行为不变。无 **BREAKING** 变更。
- 数据格式：control dataset schema 新版本要求 provenance 字段；阈值条目字段保持兼容。
- 遗留路径：`state.json` 仍是 SQLite 的投影；旧 JSON 缓存保留为只读投影/兼容层，不再作为正式写入口。
