...
def resolve_target_structure(
    ...
    *,
    ...
    configured_structure = target.get("structure") or {}
    chain_reviewed = bool(configured_structure.get("chain") or (selected or {}).get("chain"))
    coordinates_ready = bool(selected and selected_grade in {"A", "B", "C"})
    ...
    return {
        ...
        "required_next_step": (
            "review_target_chain_and_epitope_coordinates"
            if coordinates_ready and not ready_for_design
            else "inspect_epitope_and_generate_conformational_ensemble"
            if selected and (selected.get("kind") == "predicted" or selected_grade == "C")
            else "run_target_structure_prediction"
            if not selected
            else "inspect_epitope_coordinates"
        ),
    }
