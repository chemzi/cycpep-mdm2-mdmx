...
def load_control_dataset(
    ...
    *,
    ...
    """Load and validate a JSON control dataset before it reaches calibration."""
    source = Path(path)
    try:
        payload = json.loads(source.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
    ...
    controls, metadata = _coerce_dataset(payload)
    payload_meta = payload if isinstance(payload, dict) else {}
    for key in ("project_id", "approved_digest", "protocol", "schema_version"):
        if key in payload_meta and key not in metadata:
            metadata[key] = payload_meta[key]
    if project_id and metadata.get("project_id") and metadata["project_id"] != project_id:
        raise ControlDataError("control dataset project_id does not match approved project")
    if (
        approved_digest
        and metadata.get("approved_digest")
        and metadata["approved_digest"] != approved_digest
    ):
        raise ControlDataError("control dataset approved_digest does not match approved project")
    if not controls:
        raise ControlDataError("control dataset contains no control records")
...
def calibrate_thresholds(
    *,
    ...
    audit: dict[str, Any] = {
        ...
        "max_false_positive_rate": max_false_positive_rate,
        "min_positive_recall": min_positive_recall,
        "protocol_hash": _protocol_hash(protocol or dataset_meta.get("protocol")),
        "metrics": {},
        "calibrated_keys": [],
        "skipped_keys": [],
    }
