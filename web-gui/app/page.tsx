...
type ResolvedCandidate = { id?: string; uniprot?: string; name?: string; organism_id?: number; [key: string]: unknown };
type ProjectDraft = { draft_id: string; name?: string; project_id?: string; targets: { id?: string; uniprot?: string }[]; bootstrap?: { resolved_candidates?: ResolvedCandidate[]; ambiguous_identifier?: boolean; selected_candidate?: ResolvedCandidate }; review?: { blocking_issues?: string[]; warnings?: string[] } };
...
const normaliseApiBase = (value: string) => value.replace(/^http:\/\/localhost:8765(?=\/|$)/, "http://127.0.0.1:8765");
...
function useSettings() {
  ...
  useEffect(() => { try { const raw = localStorage.getItem("cycpep-studio-settings"); if (raw) { const saved = JSON.parse(raw) as Partial<Settings>; const next = { ...DEFAULT_SETTINGS, ...saved, apiBase: normaliseApiBase(String(saved.apiBase || DEFAULT_SETTINGS.apiBase)) }; setValue(next); localStorage.setItem("cycpep-studio-settings", JSON.stringify(next)); } } catch {} }, []);
...
export default function Home() {
  ...
  return <main className="workbench">
    ...
    {createOpen && <CreateDialog settings={settings} connectionId={connectionId} onClose={() => setCreateOpen(false)} onActivated={data => { setSnapshot(data); setSelectedId(""); setCreateOpen(false); notify("项目已批准并开始全自动运行"); }}/>}��m�G����ƭy�iewer-empty"><div className="empty-orbit"><i/><i/><i/></div><span>VERIFIED ARTIFACT REQUIRED</span><h2>尚无可加载的真实结构</h2><p>只有七层全清、manifest 存在且 SHA-256 匹配的 PDB/mmCIF 才会在此显示。</p></div>}{state === "loading" && <div className="viewer-message">正在验证并加载坐标…</div>}{state === "error" && <div className="viewer-message error">{message}</div>}{state === "ready" && <div className="viewer-controls">{(["cartoon", "sticks", "surface"] as const).map(item => <button key={item} className={style === item ? "active" : ""} onClick={() => applyStyle(item)}>{item}</button>)}</div>}</div>;
}
...
function ConnectionDialog({ settings, save, onClose, onConnected, notify }: { settings: Settings; save: (s: Settings) => void; onClose: () => void; onConnected: (s: Snapshot, id: string) => void; notify: (s: string) => void }) {
  ...
  const connect = async () => { setBusy(true); try { if (mode === "local") { const localBase = normaliseApiBase(apiBase); save({ ...settings, apiBase: localBase }); onConnected(await api<Snapshot>(localBase, "snapshot"), ""); } else { const result = await api<{ connection_id: string; snapshot: Snapshot }>(DEFAULT_SETTINGS.apiBase, "connections/ssh", { method: "POST", body: JSON.stringify({ host, username, port, password, workspace_root: root }) }); onConnected(result.snapshot, result.connection_id); } } catch (cause) { notify(cause instanceof Error ? cause.message : "连接失败"); } finally { setBusy(false); } };
...
function CreateDialog({ settings, connectionId, onClose, onActivated }: { settings: Settings; connectionId: string; onClose: () => void; onActivated: (s: Snapshot) => void }) {
  const [identifier, setIdentifier] = useState(""); const [kind, setKind] = useState("auto"); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [draft, setDraft] = useState<ProjectDraft | null>(null); const [selectedCandidate, setSelectedCandidate] = useState(""); const [designCount, setDesignCount] = useState(4); const [predictionCount, setPredictionCount] = useState(4); const [gpuMinutes, setGpuMinutes] = useState(360); const [rounds, setRounds] = useState(2);
  const create = async () => { setBusy(true); setError(""); try { if (!connectionId) throw new Error("请先连接 SSH 服务器"); const job = await api<{ job_id: string }>(settings.apiBase, "connections/ssh/project-drafts", { method: "POST", body: JSON.stringify({ connection_id: connectionId, identifier, identifier_type: kind, organism_id: 9606, objective: "binder" }) }); let result: { status: string; result?: ProjectDraft; error?: string } = { status: "running" }; for (let attempt = 0; attempt < 90 && result.status === "running"; attempt += 1) { await new Promise(resolve => window.setTimeout(resolve, 2000)); result = await api<{ status: string; result?: ProjectDraft; error?: string }>(settings.apiBase, "connections/ssh/project-drafts/status", { method: "POST", body: JSON.stringify({ job_id: job.job_id }) }); } if (result.status !== "complete" || !result.result) throw new Error(result.error || "靶点解析超时"); setSelectedCandidate(""); setDraft(result.result); } catch (cause) { setError(cause instanceof Error ? cause.message : "创建失败"); } finally { setBusy(false); } };
  const resolveCandidate = async () => { if (!draft || !selectedCandidate) return; setBusy(true); setError(""); try { const updated = await api<ProjectDraft>(settings.apiBase, `connections/ssh/project-drafts/resolved-candidate`, { method: "POST", body: JSON.stringify({ connection_id: connectionId, draft_id: draft.draft_id, candidate_ref: selectedCandidate }) }); setDraft(updated); setSelectedCandidate(""); } catch (cause) { setError(cause instanceof Error ? cause.message : "候选选择失败"); } finally { setBusy(false); } };
