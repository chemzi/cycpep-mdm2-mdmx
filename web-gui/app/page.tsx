  ...
  return <main className="workbench">
    ...
    <section className="workbench-main">
      <aside className="agent-rail">
        ...
        <div className="candidate-rail"><div className="section-label"><span>CANDIDATES</span><em>{snapshot?.candidates.length || 0}</em></div><input placeholder="搜索候选…" aria-label="搜索候选"/><div className="candidate-list">{snapshot?.candidates.slice(0, 12).map(row => { const hasArtifact = Boolean(row.artifact_id); const cleared = row.all_layers_pass; return <button key={row.candidate_id} className={`${selectedId === row.candidate_id ? "active" : ""}${hasArtifact ? " has-artifact" : ""}${hasArtifact && !cleared ? " uncleared" : ""}`.trim()} onClick={() => setSelectedId(row.candidate_id)} title={hasArtifact ? (cleared ? "坐标已注册 · 七层全清" : "坐标已注册 · 未全清，仅供参考") : "无坐标 artifact"}><i className={`candidate-dot${hasArtifact ? " on" : ""}`}/><div><b>{row.candidate_id}</b><code>{row.sequence}</code></div><span className={hasArtifact && !cleared ? "warn" : ""}>{row.layers.filter(Boolean).length}/7</span></button>; })}{!snapshot?.candidates.length && <p>数据层中没有候选</p>}</div></div>
      </aside>

      <section className="structure-stage">
        <div className="stage-toolbar"><div><span>STRUCTURE WORKSPACE</span><b>{selected?.candidate_id || "No candidate selected"}</b></div><select value={selectedId} onChange={event => setSelectedId(event.target.value)} disabled={!snapshot?.candidates.length}><option value="">选择候选</option>{snapshot?.candidates.map(row => <option key={row.candidate_id}>{row.candidate_id}</option>)}</select><div className="stage-meta"><span>{selected?.source_route || "route 未记录"}</span><span className={selected?.artifact_id ? (selected.all_layers_pass ? "" : "warn") : ""}>{selected?.artifact_id ? (selected.all_layers_pass ? "HASH VERIFIED" : "HASH VERIFIED · NOT CLEARED") : "NO ARTIFACT"}</span></div></div>
        <StructureViewer candidate={selected} apiBase={settings.apiBase}/>
        <div className="structure-status"><span><i className="receptor"/>Receptor</span><span><i className="peptide"/>Cyclic peptide</span><em>{selected?.artifact_id ? `artifact ${selected.artifact_id}` : "候选暂无可加载坐标 artifact"}</em></div>
...
function StructureViewer({ candidate, apiBase }: { candidate: Candidate | null; apiBase: string }) {
  ...
  return <div className="viewer-shell"><div ref={element} className="viewer-canvas"/>{state === "idle" && <div className="viewer-empty"><div className="empty-orbit"><i/><i/><i/></div><span>NO COORDINATES AVAILABLE</span><h2>尚无可加载的真实结构</h2><p>候选暂未注册经过哈希校验的坐标 artifact。已注册坐标的候选（无论七层是否全清）都可在此点选后加载。</p></div>}{state === "loading" && <div className="viewer-message">正在验证并加载坐标…</div>}{state === "error" && <div className="viewer-message error">{message}</div>}{state === "ready" && <div className="viewer-controls">{(["cartoon", "sticks", "surface"] as const).map(item => <button key={item} className={style === item ? "active" : ""} onClick={() => applyStyle(item)}>{item}</button>)}</div>}{state === "ready" && candidate?.artifact_id && !candidate.all_layers_pass && <div className="viewer-notice">NOT CLEARED · 仅供参考（仅通过 manifest 身份与 SHA-256 校验，七层未全清）</div>}</div>;
}
...
function BottomDock({ tab, snapshot, apiBase }: { tab: BottomTab; snapshot: Snapshot | null; apiBase: string }) {
  ...
  if (tab === "Artifacts") { const rows = snapshot?.candidates.filter(row => row.artifact_id) || []; return <div className="artifact-table">{rows.map(row => <div key={row.candidate_id}><b>{row.candidate_id}</b><code>{row.artifact_id}</code><em className={row.all_layers_pass ? "" : "warn"}>{row.all_layers_pass ? "CLEARED" : "NOT CLEARED"}</em><span>SHA-256 verified</span><a href={join(apiBase, `artifacts/${row.artifact_id}/coordinates`)} download>下载坐标</a></div>)}{!rows.length && <div className="dock-empty"><b>NO VERIFIED ARTIFACTS</b><span>没有经过 manifest 身份与 SHA-256 校验的坐标 artifact。</span></div>}</div>; }
