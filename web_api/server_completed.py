...
def _planner_cost_payload(events: list[dict]) -> list[dict]:
    """Project planner cost evidence into a stable UI-facing shape.

    The raw evidence stream remains available via ``recent_evidence``; this
    projection avoids duplicating parsing logic in the web GUI.
    """
    costs = []
    for event in events:
        if not isinstance(event, dict) or event.get("event_type") != "planner_plan":
            continue
        payload = event.get("payload")
        if not isinstance(payload, dict):
            # EvidenceEvent.to_dict() may flatten payload fields depending on
            # contract version; support both shapes for compatibility.
            payload = event

        resource_requests = payload.get("resource_requests")
        ...
        for request in resource_requests:
            ...
            tasks.append({
                ...
                "route": route,
                "resource_class": request.get("class"),
                "estimated_gpu_minutes": request.get("estimated_gpu_minutes"),
                "estimated_gpu_hours": request.get("estimated_gpu_hours"),
                "timeout_seconds": request.get("timeout_seconds"),
                "estimate_status": request.get("estimate_status"),
                "cost_model_version": request.get("cost_model_version"),
            })
        ...
        costs.append({
            ...
            "estimated_gpu_minutes": payload.get("estimated_gpu_minutes"),
            "estimated_gpu_hours": payload.get("estimated_gpu_hours"),
            "budget_status": payload.get("budget_status") or {},
...
def local_snapshot() -> dict:
    ...
    return {
        ...
        "planner_costs": _planner_cost_payload(evidence[-100:]),
...
print(json.dumps({{"state":State.load(),"rows":CandidateIndex.load(),"events":EvidenceLogger.get_all()[-100:]}},ensure_ascii=False))'''
    ...
    return {
        ...
        "recent_evidence": payload["events"],
        "planner_costs": _planner_cost_payload(payload["events"]),
        "integrity_warnings": warnings,
    }
